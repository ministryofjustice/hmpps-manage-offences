...
export default class PartsAndOffencesRoutes {
  ...
  GET_PROGRESSION_MODEL_EXCLUSION_LISTS = async (req: Request, res: Response): Promise<void> => {
    const progressionModelExclusionLists = await this.offenceService.getProgressionModelExclusionLists(res.locals.user)
    res.render('pages/schedules/progressionModelExclusionLists', {
